...
- `internal/musicsource` is the port for reading a listener's existing playlists from a streaming service; `internal/musicsource/tidal` (OAuth device flow, approved in the system browser), `internal/musicsource/qobuz` (web-player token captured from the embedded sign-in window), and `internal/musicsource/spotify` (web-player token captured the same way; no refresh, so an expired session reports as disconnected) are reverse-engineered adapters using community credentials and undocumented endpoints, and `internal/musicsource/fake` drives the import pipeline in tests.
